package ao.learn.mst.gen5.example.kuhn

import ao.learn.mst.gen5.example.kuhn.action.{KuhnCheckFold, KuhnCallRaise, KuhnActionSequence}
import ao.learn.mst.gen5.example.kuhn.card.KuhnDeck
import ao.learn.mst.gen5.example.kuhn.state.terminal.KuhnTerminalStatus
import ao.learn.mst.gen5.example.kuhn.state.{KuhnPosition, KuhnState}
import util.Random
  ...
  "Initial state" should {
    ...
    "First player can check or raise" in {
      ...
      initialState.availableActions must equalTo(
        Seq(KuhnCheckFold, KuhnCallRaise))
    }
    ...
    "On first player checks" in {
      val onFirstPlayerChecks =
          initialState.act(KuhnCheckFold)
      ...
      "On second player checks" in {
        val onSecondPlayerChecks =
            onFirstPlayerChecks.act(KuhnCheckFold)
        ...
        "On second player raises" in {
          val onSecondPlayerRaises =
            onFirstPlayerChecks.act(KuhnCallRaise)
          ...
          "On first player folds" in {
            val onFirstPlayerFolds =
              onSecondPlayerRaises.act(KuhnCheckFold)
          ...
          "On first player calls" in {
            val onFirstPlayerCalls =
              onSecondPlayerRaises.act(KuhnCallRaise)
    ...
    "On first player raises" in {
      val onFirstPlayerRaises =
        initialState.act(KuhnCallRaise)
      ...
      "On player two folds" in {
        val onLastPlayerFolds =
          onFirstPlayerRaises.act(KuhnCheckFold)
      ...
      "On player two calls" in {
        val onLastPlayerCalls =
          onFirstPlayerRaises.act(KuhnCallRaise)
